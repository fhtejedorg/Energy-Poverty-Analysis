...
dat_COROP <- read.delim("data/wijken/Gebieden_in_Nederland_2023_85385NED.csv",sep = ";", header = T)
...
plt
ggsave(plot = plt, filename = file.path(outpath_figure, "fig_summary_urban.png"), dpi=300, width = 6.5, height = 4, units = "in")  # Good default for LaTeX)
...
plot(g_def_wijken_23$p_pop_Origin_Outside_Europe_Born_Outside_NL, log(g_def_wijken_23$LIHE))
...
ggplot(g_def_provincie) +
  ...
  scale_fill_brewer(palette = "Set3", name = "Province") +  # 12-color palette (use "Paired", "Set3", etc.)
  theme_minimal() +
...
#### Export files to be used in ArcGIS to create the maps with nicer legends
...
### Output to --> output/R/data/03_Spatial Data Processing
